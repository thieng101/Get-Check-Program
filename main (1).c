  ...
  if (c <= 25) {
    for (int i = 0; i < c; i++) {
      ...
      if (n <= 500000) {
        ...
        int currTime = 0;
        for (int m = 0; m < n; m++) {
          // currTime(reference will update currTime)
          ...
          printf("%s from line %d checks out at time %d.", tmpPtr->name,
...
customer *createCustomer() {
  ...
  scanf("%d %d %s %d", &customer->timeEnteringLine, &customer->lineNumber, name,
        &customer->numOfTickets); // check condition for each value
...
int getNext(queue lines[], int *currTime) {
  ...
  // declare bestime before we begin the comparison process
  int bestTime = MAXTIME + 1;

  // First just look for active lines. (Go in order to break ties.)
  for (int i = 0; i < NUMLINES; i++) {

    // check if the current line if empty
    if (empty(&lines[i]) == 1)
      continue;
    // if that is the case simply continue to the next iteration
